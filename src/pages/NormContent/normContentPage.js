...
import AnchorMenu from '../../components/Layout/AnchorMenu/anchorMenu';
import Micropanel from '../../components/Layout/MicroPanel/microPanel';  // Asegúrate de importar Micropanel
import Loading from '../../components/Layout/Loading/loading';
import { useArticles } from '../../hooks/useArticles';
import { parseXmlToReact } from '../../services/xmlParser';
import Dropdown from '../../components/Layout/DropDown/dropdown';
import ToggleButton from '../../components/Layout/ToggleButton/toggleButton';
import './normContentPage.css';

const NormContentPage = () => {
  const { content, error, articleTitles, normTitle } = useArticles(); // Asume que useArticles ahora también extrae normTitle
  const [activeArticleIndex, setActiveArticleIndex] = useState(null);  
  const [hoveredArticle, setHoveredArticle] = useState(null); // Estado para el artículo actualmente destacado
  ...
  return (
    <div className='norma'>
      ...
      <div className="body-content">
        <h1>{normTitle}</h1> {/* Muestra el título de la norma */}
        <AnchorMenu 
        ...
        {content ? (
          <div className='norma-content'>
            {parseXmlToReact(content, hoveredArticle, setHoveredArticle)} {/* Modifica para pasar el estado hovered */}
          </div>
        ) : (
          <Loading />
        )}
